...
h(k,1)=h(1);
   ...
   disp(t(k,1))
    ...
    if k==1
        ...
        while j<i
            h_sp=6.5;                                          %% put setpoint as 1.2 to 1.4 times the maximum height (or steady state height) 
            e(j)=h_sp-h(k,j);
            if j==1
                 u_feedback(j)=Kp*(e(j))+Kp*(e(j)/Ti)+1;
            else
               u_feedback1(j)=Kp*(e(j));        
              %e(j)=e(j)+e(j-1);
            u_feedback(j)=u_feedback1(j)+Kp*((e(j)+e(j-1))/Ti)+1;
...
figure(4);plot(t(k-1,1:q-1),u_feedback,'b');legend('controller input');
%figure(5);plot(t(k-1,1:q-1),u_plant,'g');legend('plant input');title('control signal');
